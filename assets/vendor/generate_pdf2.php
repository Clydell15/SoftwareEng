...
try {
    ...
    $pdf->SetTitle('TaskFlow Export');
    $pdf->SetSubject('Task Management Export');

    // Set default header data
    $pdf->SetHeaderData('', 0, 'TaskFlow Export', 'Generated on ' . date('Y-m-d H:i:s'));
    ...
    $html = '<h1 style="color: #2c3e50; text-align: center;">TaskFlow Export</h1>';
...
function generateTodoContent($options, $conn, $userId) {
    ...
    $html .= '<thead style="background-color: #ecf0f1;">';
    $html .= '<tr>';
    $html .= '<th style="width: 40%;">Task</th>';
    $html .= '<th style="width: 15%;">Status</th>';
    
    if ($options['include_difficulty'] ?? true) {
        $html .= '<th style="width: 15%;">Difficulty</th>';
    }
    if ($options['include_due_dates'] ?? true) {
        $html .= '<th style="width: 15%;">Due Date</th>';
    }
    if ($options['include_tags'] ?? true) {
        $html .= '<th style="width: 15%;">Tags</th>';
    }
    ...
    foreach ($tasks as $task) {
        ...
        if ($options['include_difficulty'] ?? true) {
            ...
            // Check for difficulty_numeric first (the actual numeric value)
            if (isset($task['difficulty_numeric']) && $task['difficulty_numeric'] > 0) {
                $difficulty = number_format($task['difficulty_numeric'], 1);
            } 
            // Fallback to difficulty_level if difficulty_numeric is not available
...
function generateCompletedContent($options, $conn, $userId) {
    ...
    if (empty($selectedIds)) {
        $sql = "SELECT 
                    t.id, t.title, t.description, t.status, t.difficulty_level, t.difficulty_numeric,
                    t.due_date, t.created_at,
                    GROUP_CONCAT(tags.name SEPARATOR ', ') as tag_names
                FROM tasks t
                LEFT JOIN task_tags tt ON t.id = tt.task_id
...
// Build table
$html .= '<table border="1" cellpadding="5" cellspacing="0" style="width: 100%; border-collapse: collapse;">';
$html .= '<thead style="background-color: #d5f4e6;">';
$html .= '<tr>';
$html .= '<th style="width: 60%;">Task</th>';
$html .= '<th style="width: 20%;">Completed Date</th>';

if ($options['include_difficulty'] ?? true) {
    $html .= '<th style="width: 20%;">Difficulty</th>';
}
...
$html .= '</thead><tbody>';
    ...
    foreach ($tasks as $task) {
        ...
        // Completed Date
        $completedDate = $task['updated_at'] ? date('M j, Y g:i A', strtotime($task['updated_at'])) : 'Unknown';
...
function generateCategoriesContent($options, $conn, $userId) {
    ...
    $html .= '<tr>';
    $html .= '<th style="width: 30%;">Category Name</th>';
    $html .= '<th style="width: 70%;">Associated Tasks</th>';
    $html .= '</tr>';
    $html .= '</thead><tbody>';
...
function generateArchiveContent($options, $conn, $userId) {
    ...
    // Get archived tasks
    $taskSql = "SELECT title, description, updated_at FROM tasks WHERE user_id = ? AND archived = 1 ORDER BY updated_at DESC";
    ...
    if ($archivedTasks !== false && !empty($archivedTasks)) {
        $html .= '<h3 style="color: #d35400;">Archived Tasks</h3>';
        ...
        foreach ($archivedTasks as $task) {
            ...
            $html .= ' <em>(Archived on: ' . date('M j, Y', strtotime($task['updated_at'])) . ')</em></li>';
        }
        $html .= '</ul>';
    }
    
    if ($archivedCategories !== false && !empty($archivedCategories)) {
        $html .= '<h3 style="color: #d35400;">Archived Categories</h3>';
        $html .= '<ul style="padding-left: 20px;">';
